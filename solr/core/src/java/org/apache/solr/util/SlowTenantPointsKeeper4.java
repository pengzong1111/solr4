...
public class SlowTenantPointsKeeper4 {
...
//1. we think of each core as a tenant to start off. Each core has its default points. 
  private static SlowTenantPointsKeeper4 tenantPointsKeeper = new SlowTenantPointsKeeper4();
  ...
  public static SlowTenantPointsKeeper4 getInstance() {
    return tenantPointsKeeper;
  }
  
  private SlowTenantPointsKeeper4() {
    try {
  ...
  class ActualThroughputChecker extends Thread {
    ...
    public void run() {
      ...
      while (true) {
        ...
        if(count > 10) { // to avoid too aggressively adjust quota prematurely during bootstrap
          checkAndAdjustSpeedupTenant(); // check if there is any slow tenant getting faster, if so we need to improve its quota by getting some tokens from those with larger quota
        }
      }
    }

    private void checkAndAdjustSpeedupTenant(/*Map<String,List<Integer>> tenantToRecentActualThroughputsMap*/) {
      if(tenantToRecentActualThroughputsMap == null) {
        pw.println("WARNING: tenantToRecentActualThroughputsMap is null" );
        return;
      }
      
    /*  if(tenantToRecentActualThroughputsMap.size() < cacheSize) {
        pw.println("WARNING: no enough history records in tenantToRecentActualThroughputsMap: " + tenantToRecentActualThroughputsMap.size());
        return;
      } else if(tenantToRecentActualThroughputsMap.size() > cacheSize) {
        pw.println("WARNING: too many history records in tenantToRecentActualThroughputsMap: " + tenantToRecentActualThroughputsMap.size());
        return;
      }*/
      ...
      for(Entry<String,List<Integer>> entry : tenantToRecentActualThroughputsMap.entrySet()) {
        String coreName = entry.getKey();
        List<Integer> recentThroughputs = entry.getValue();
    //    List<Integer> latestThroughputs = tenantToRecentActualThroughputsMap.get(coreName);
        if(recentThroughputs == null) {
          ...
          for(int i=1; i<cacheSize; i++) {
            if(recentThroughputs.get(i) >= 1.15*recentThroughputs.get(i-1) ) {
      ...
      if(speedupTenants.size() == 0) {
        pw.println("count: " + count + " No increasing tenants found"); pw.flush();
      } else {
        int threshold = MAXIMUM_LOGICAL_THROUGHPUT/coreNameToTenantMap.size();
        for(String tenant: speedupTenants) {
          TenantTokenInfo tenantInfo = coreNameToTenantMap.get(tenant);
          if(tenantInfo.quota < threshold && 1.15*tenantInfo.quota <= threshold) {
            decrementLargerQuotaTenants(tenantInfo.quota, 0.15);
            tenantInfo.quota = (int) (tenantInfo.quota*1.15);
          }
        }
      }
    }

    private void decrementLargerQuotaTenants(int quota, double percentage) {
      double overall2Decrement = quota * percentage;
      List<String> largerQuotaTenants = new LinkedList<String>();
      for(Entry<String, TenantTokenInfo> entry : coreNameToTenantMap.entrySet()) {
        TenantTokenInfo info = entry.getValue();
        if(info.quota > quota) {
          largerQuotaTenants.add(entry.getKey());
        }
      }
      
      if(largerQuotaTenants.size() == 0) {
        pw.println("count " + count + ": no tenants has quota larger than " + quota);
      } else {
        int decrement = (int) (overall2Decrement/largerQuotaTenants.size());
        
        for(String tenant: largerQuotaTenants) {
          int largeQuota = coreNameToTenantMap.get(tenant).quota;
          if((largeQuota - decrement) >= 0.98 * MAXIMUM_LOGICAL_THROUGHPUT/coreNameToTenantMap.size()) {
            coreNameToTenantMap.get(tenant).quota = largeQuota - decrement;
          }
        }
      }
